...
<!-- menu -->
	<ul id="menu">
		...
		<li><a
			href="Controller?command=localization&local=en&page=reportoderbyuser"><img
				src="images/eng.png" /></a></li>
		<li><a
			href="Controller?command=localization&local=ru&page=reportoderbyuser"><img
...
<form  action="Controller" method="post">   
      ...
      <button type="submit">${my_archive}</button>  
  </form>
  
<p align="center">
		<b><c:out value="${my_oders}" /></b>
	</p>
				...
				<c:forEach var="oder" items="${requestScope.oders}">
					...
					<tr>
						...
						<td>
							...
							<a href='<c:url value="/Controller?command=canceloderbyuser&link_id_oder=${oder.id}" />'>${my_cancel}</a>
	...
	<div class="pagination">
		<c:if test="${currentPage != 1}">
			<a href="Controller?command=reportoderbyuser&viewrecordsPerPage=${recordsPerPage}&tPage=${currentPage-1}">&laquo;</a>
		</c:if>
		...
		<c:forEach begin="1" end="${noOfPages}" var="i">
			<c:choose>
				...
				<c:otherwise>
					<a href="Controller?command=reportoderbyuser&recordsPerPage=${recordsPerPage}&tPage=${i}">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>

		<c:if test="${currentPage lt noOfPages}">
			<a href="Controller?command=reportoderbyuser&recordsPerPage=${recordsPerPage}&tPage=${currentPage+1}">&raquo;</a>
		</c:if>
	</div>
