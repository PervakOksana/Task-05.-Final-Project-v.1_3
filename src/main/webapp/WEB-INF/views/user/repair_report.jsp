...
<!-- menu -->
	<ul id="menu">
		...
		<li><a
			href="Controller?command=localization&local=en&page=reportrepairbyuser"><img
				src="images/eng.png" /></a></li>
		<li><a
			href="Controller?command=localization&local=ru&page=reportrepairbyuser"><img
...
<div style="text-align: center;">
				...
				<c:forEach var="repairBill" items="${requestScope.repairBill}">
					...
					<tr>
						...
						<td>
							...
							<a href='<c:url value="/Controller?command=topayrepairbillpage&link_id_repair=${repairBill.id}" />'>${pay}</a>
	...
	<div class="pagination">
		<c:if test="${currentPage != 1}">
			<a href="Controller?command=reportrepairbyuser&viewrecordsPerPage=${recordsPerPage}&tPage=${currentPage-1}">&laquo;</a>
		</c:if>
		...
		<c:forEach begin="1" end="${noOfPages}" var="i">
			<c:choose>
				...
				<c:otherwise>
					<a href="Controller?command=reportrepairbyuser&recordsPerPage=${recordsPerPage}&tPage=${i}">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>

		<c:if test="${currentPage lt noOfPages}">
			<a href="Controller?command=reportrepairbyuser&recordsPerPage=${recordsPerPage}&tPage=${currentPage+1}">&raquo;</a>
		</c:if>
	</div>
