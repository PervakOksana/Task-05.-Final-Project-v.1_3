...
<fmt:message bundle="${loc}" key="local.find.page.cancel" var="cancel" />
<fmt:message bundle="${loc}" key="local.find.page.send" var="send" />
<fmt:message bundle="${loc}" key="local.find.page.user.empty" var="user_empty" />
...
<body>
	...
	<!-- menu -->
	<ul id="menu">
		<li><a href="Controller?command=choosecarview"><c:out
					value="${home}" /></a></li>
		...
		<li><a
			href='<c:url value="/Controller?command=tologinationpage" />'>${login}</a></li>
		<li><a
			href="Controller?command=localization&local=en&page=tofindpage"><img
				src="images/eng.png" /></a></li>
		<li><a
			href="Controller?command=localization&local=ru&page=tofindpage"><img
				src="images/ru.png" /></a></li>

		<c:if test="${sessionScope.roleByLogin eq \"USER\" }">
			<li><a><c:out value="${sessionScope.message}" /></a></li>
			<li><a href="Controller?command=logout"><c:out
						value="${logout}" /></a></li>
		</c:if>
	...
	<c:if test="${sessionScope.notice_user_empty eq \"error_user_empty\" }">
		<li><a><p align="center">
		<b><c:out value="${user_empty}" /></b>
	        </p></li>
	...
	<c:if test="${sessionScope.notice_user eq \"error_user\" }">
		<li><a><p align="center">
		<b><c:out value="${send}" /></b>
	</p></a></li>
		<form action="Controller" method="post">
			<input type="hidden" name="command" value="choosecarview" />
			<div class="container" style="background-color: #f1f1f1">
