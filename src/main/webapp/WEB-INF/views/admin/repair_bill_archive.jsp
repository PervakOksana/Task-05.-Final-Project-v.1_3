...
<fmt:message bundle="${loc}" key="local.locbutton.name.en"
    ...
    <fmt:message bundle="${loc}" key="local.cancel.oder.page.completed" var="completed" />
...
<!-- menu -->
	<ul id="menu">
		...
		<li><a
			href="Controller?command=localization&local=en&page=addrepairbillview&take=archive"><img
				src="images/eng.png" /></a></li>
		<li><a
			href="Controller?command=localization&local=ru&page=addrepairbillview&take=archive"><img
...
<p align="center">
			...
			<div style="text-align: center;">
				...
				<c:forEach var="oder" items="${requestScope.oders}">
					...
					<tr>
						...
						<td>
							<a href='<c:url value="/Controller?command=torepairbillpage&id=${oder.id}" />'>${create}</a>	
						</td>
						<td>
						 <a href='<c:url value="/Controller?command=completedorder&id=${oder.id}" />'>${completed}</a>
                        </td>
					</tr>
	...
	<div class="pagination">
		<c:if test="${currentPage != 1}">
			<a href="Controller?command=addrepairbillview&take=archive&viewrecordsPerPage=${recordsPerPage}&tPage=${currentPage-1}">&laquo;</a>
		</c:if>
		...
		<c:forEach begin="1" end="${noOfPages}" var="i">
			<c:choose>
				...
				<c:otherwise>
					<a href="Controller?command=addrepairbillview&take=archive&recordsPerPage=${recordsPerPage}&tPage=${i}">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>

		<c:if test="${currentPage lt noOfPages}">
			<a href="Controller?command=addrepairbillview&take=archive&recordsPerPage=${recordsPerPage}&tPage=${currentPage+1}">&raquo;</a>
		</c:if>
	</div>
