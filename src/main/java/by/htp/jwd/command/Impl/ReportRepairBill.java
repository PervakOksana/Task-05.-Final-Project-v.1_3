...
public class ReportRepairBill implements Command {

	private final Logger LOGGER = LoggerFactory.getLogger(ReportRepairBill.class);
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (request.getParameter(CommandField.ONLY_NOT_PAY) != null) {
				repairBills = repairBillService.repairBillNotPay(currentPage);
			}
			if (request.getParameter(CommandField.ONLY_NOT_PAY) == null) {
				repairBills = repairBillService.allRepairBill(currentPage);
			}
			...
			request.setAttribute(CommandField.REPAIR_BILL, repairBills);

			RequestDispatcher requestDispather = request.getRequestDispatcher(CommandField.PATH_ADD_REPAIR_BILL);
			requestDispather.forward(request, response);

		} catch (ServiceException e) {
			LOGGER.error("ReportRepairBill::ServiceException.");
